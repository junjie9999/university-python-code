...
m=9.10938356e-31
hbar= 1.055E-34
#DelV=1/2*(k*(x)**2)#hamlitonian potential energy
#DelV=1
#Matrix elements of the kinetic energy operator
def KE(i,j):
    if i==j:
       ke = (1/2)*(i*pi/L)**2# kinetic term of the Hamiltonian in Hartree
    else:
       ke=0
    return ke
...
def steppot(x):
    ...
    if abs(x) <= L/2:
       ...
       fn = 0# this is for the condition when it goes to infinity out side the potential 
    return fn
...
def V(i,j):
    ...
    result = quad(integrand, -L/2, L/2, args=(i,j)) #integration of wavefunctions due to the integration only need for the wave function and potentail energy  
    return result[0]
...
for state in range(0,5): #print for the first five state manually  
  del xdata[:]
  del eigfunc[:]
